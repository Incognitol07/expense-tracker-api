...
from app.models import User, Budget, Notification
from app.routers import auth_router, expenses_router, categories_router, budget_router, analytics_router, alerts_router, admin_router, notifications_router, groups_router, debt_router, profile_router
...
def check_and_deactivate_expired_budgets():
    ...
    try:
        ...
        budgets = db.query(Budget).filter(Budget.status == 'active').all()
        ...
        for budget in budgets:
            if budget.end_date and budget.end_date < datetime.now().date():
                ...
                logger.info(f"Budget (ID: {budget.id}) for user {budget.user_id} has been deactivated.")
                ...
                if not existing_notification:
                    logger.info(f"Creating notification for user ID: {budget.user_id} with message: '{message}'")
                    ...
                    logger.info(f"User ID: {budget.user_id} already has an unread notification with the same message.")
                logger.info(f"Deactivation notification sent for user ID: {budget.user_id}")
    finally:
        db.close()
