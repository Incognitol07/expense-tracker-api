...
def set_budget(budget_data: BudgetCreate, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
...
def get_budget(db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
...
def update_budget(budget_data: BudgetUpdate, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    budget = db.query(Budget).filter(Budget.user_id == user["id"]).first()
    if not budget:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Budget not set.")
...
def get_budget_status(db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    budget = db.query(Budget).filter(Budget.user_id == user["id"]).first()
    if not budget:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Budget not set.")
    ...
    return BudgetStatus(remaining_amount=remaining_amount, limit=budget.amount_limit, period={"start_date": budget.start_date, "end_date": budget.end_date})

@router.get("/budget/history", response_model=list[BudgetHistory])
def get_budget_history(db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
