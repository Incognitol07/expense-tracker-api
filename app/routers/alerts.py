...
from app.models import User, Alert, Expense, GeneralBudget
...
async def check_thresholds(user_id: int):
    ...
    db = (
        SessionLocal()
    )  # Assuming SessionLocal is the session factory from your database setup
    try:
        ...
        if adherence > 0:
            logger.info(
                f"Threshold exceeded for user ID {user_id}. Exceedance amount: {adherence:.2f}"
            )
...
def create_alert(
    ...
    """
    logger.info(
        f"Attempting to create an alert for user '{current_user.username}' (ID: {current_user.id})"
    )
    # Check if an alert with the same threshold already exists for the user
    existing_alert = (
        db.query(Alert)
        .filter(
            Alert.user_id == current_user.id, Alert.threshold == alert_data.threshold
        )
        .first()
    )
    ...
    if existing_alert:
        logger.warning(
            f"Alert with threshold {alert_data.threshold} already exists for user '{current_user.username}' (ID: {current_user.id})"
        )
        raise HTTPException(
    ...
    active_budget = (
        db.query(GeneralBudget)
        .filter(
            GeneralBudget.user_id == current_user.id, GeneralBudget.status == "active"
        )
        .first()
    )
    if not active_budget:
        logger.warning(
            f"No active budget found for user '{current_user.username}' (ID: {current_user.id}) while creating alert"
        )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    db.refresh(alert)
    logger.info(
        f"Alert created successfully for user '{current_user.username}' (ID: {current_user.id}) with threshold: {alert_data.threshold}"
    )
    ...
    logger.info(
        f"Background task scheduled to check thresholds for user '{current_user.username}' (ID: {current_user.id})"
    )
    return alert
...
def get_alerts(
    ...
    """
    logger.info(
        f"Fetching all alerts for user '{current_user.username}' (ID: {current_user.id})"
    )
...
def update_alert(
    ...
    """
    logger.info(
        f"Attempting to update an alert for user '{current_user.username}' (ID: {current_user.id})"
    )
    # Retrieve the alert to be updated
    alert = db.query(Alert).filter(Alert.user_id == current_user.id).first()
    if not alert:
        logger.warning(
            f"Alert not found for user '{current_user.username}' (ID: {current_user.id})"
        )
    ...
    if not active_budget:
        logger.warning(
            f"No active budget found for user '{current_user.username}' (ID: {current_user.id}) while updating alert"
        )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    db.commit()
    logger.info(
        f"Marking all unread alert-related notifications as read for user '{current_user.username}' (ID: {current_user.id})"
    )
    ...
    db.refresh(alert)
    logger.info(
        f"Alert updated successfully for user '{current_user.username}' (ID: {current_user.id})"
    )
    background_tasks.add_task(check_thresholds, current_user.id)
    logger.info(
        f"Background task scheduled to check thresholds for user '{current_user.username}' (ID: {current_user.id})"
...
def delete_alert(
    ...
    """
    logger.info(
        f"Attempting to delete an alert for user '{current_user.username}' (ID: {current_user.id})"
    )
    ...
    db.commit()
    logger.info(
        f"Alert deleted successfully for user '{current_user.username}' (ID: {current_user.id})"
    )
    return {"detail": f"Alert of threshold {alert.threshold} deleted successfully"}
